...
class ComicDownloader:
    ...
    def search_comic_by_tencent(self, title) -> list[ComicData]:
        ...
        while (
            driver.find_element(By.CLASS_NAME, "mlm-status-loading").get_attribute(
                "style"
            )
            != "display: none;"
        ):
        ...
        for i in range(len(comic_title)):
    ...
    def search_comic_by_bing(self,title) -> list[ComicData]:
        ...
        '''
        driver=self._get_webdriver()
        driver.get('https://cn.bing.com/search?q='+title+'%20腾讯动漫')
        logging.info('搜索: '+'https://cn.bing.com/search?q='+title+'%20腾讯动漫')
        time.sleep(1)
    ...
    def _download(self,chapter:ChapterInfo,file_name:str,url:str):
